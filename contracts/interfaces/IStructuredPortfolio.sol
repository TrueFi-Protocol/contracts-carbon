...
struct TrancheData {
    /// @dev The APY expected to be granted at the end of the portfolio Live phase
    uint128 targetApy;
...
interface IStructuredPortfolio is IAccessControlUpgradeable {
     ...
     * @dev
     * - reverts if there are any active loans,
     ...
     * @dev can be executed only by tranche stored under trancheIndex
     ...
     * - can be executed only by StructuredPortfolio manager
     * - can be executed only in Live status
     * @param loanId Id of a loan that should be started
     */
     ...
     * @param loanId Id of a loan that should be repaid
     */
