 ...
 */
contract StructuredPortfolioFactoryMock is AccessControlEnumerable {
        ...
        returns (TrancheInitData[] memory trancheInitData, ITrancheVault[] memory tranches)
    {
        ...
        for (uint256 i = 0; i < tranchesCount; i++) {
            ...
            IDepositController depositController = IDepositController(
                address(new ProxyWrapper(trancheData.depositControllerImplementation, trancheData.depositControllerInitData))
            );

            IWithdrawController withdrawController = IWithdrawController(
                address(new ProxyWrapper(trancheData.withdrawControllerImplementation, trancheData.withdrawControllerInitData))
            );

            ITransferController transferController = ITransferController(
                address(new ProxyWrapper(trancheData.transferControllerImplementation, trancheData.transferControllerInitData))
            );
